...
$(document).ready(function(){
  ...
  $(document).on("click",".recommendList>li>a,.nationList>li>a,#searchList>li>a",function(){
    var currentScroll = $(document).scrollTop();
    var name = $(this).parent().find("p span").text();
    $('#movie_specification').css({"top":currentScroll + "px"});
    $('#movie_specification').show('slow');
    getMovieSpecification(name);
  });
  $(document).on("click",".boxOfficeContainer p",function(){
...
function menuItemClick(){
  ...
  if($(this).text()=="추천 영화"){
    ...
    $('.title').html("영화 통합<br>검색");
    $('.search').removeClass('hidden');
  }else if($(this).text()=="국가별 영화"){
    $('.title').html("국가별<br>영화");
...
function boxOfficeCallback(data){
  ...
  for(var i = 0 ; i < 10; i ++){
    ...
    tablerows[i].getElementsByTagName('td')[0].innerHTML = boxOfficeArray[i].rank + "(" + changedSymbol + changedvalue + ")";
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      for(var i = 0 ; i < nationCodes.length; i++){
        ...
        nationObject.name = name;
        var newOption = document.createElement('option');
        newOption.value = nationObject.code;
        newOption.text = nationObject.name;
        if(nationObject.name=="한국"){
          newOption.selected = true;
        }
